    ...
    public class AnimalShelterMappingProfile: Profile
    {
        ...
        public AnimalShelterMappingProfile()
        {
           ...
           .ForMember(so => so.Value, m => m.MapFrom(b => b.Id));
            ...
            CreateMap<Adoption, AdoptionsHistoryDto>()
                .ForMember(ah => ah.Information, m => m.MapFrom(i => $"Adopcja nr: {i.Id} z dnia {i.Date} Zwierzak nr: {i.Animal_Id} {i.Animal.Name} adoptowany przez {i.Candidate.Name}"));
